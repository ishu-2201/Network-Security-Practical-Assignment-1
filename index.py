...
# Different plaintexts with different hamming distances:


# Returns the Hamming Distances for given DES rounds of distict two keys and one single plain text
def helperfunchd(rounds1, rounds2, iham_dist):
    ans = [iham_dist]
    for x in range(16):
        ham_dist = 0
        for y in range(2):
            for z in range(32):
                if rounds1[x][y][z] != rounds2[x][y][z]:
                    ham_dist = ham_dist+1
        ans.append(ham_dist)
    return ans
...
def drawplot(key, texts):
    df = pd.DataFrame()
    for j in range(1, 6):
        r1, rounds1 = d.encrypt(key, texts[0])
        r2, rounds2 = d.encrypt(key, texts[j])
        ans = helperfunchd(rounds1, rounds2, j)
        df['HD'+str(j)] = ans
    df = df.T
    boxplot = df.boxplot()
...
key_var = ''.join(random.choice(string.ascii_lowercase) for j in range(8))
# Distict plain text with distinct Hamming Distance
texts_var = ['Hello da', 'Hello ea', 'Hfllo da',
             'Hfllo ea', 'Jfllo ea', 'Kfllo ea']
#   x           1 HD        2 HD.      3 HD.        4 HD        5 HD     -> corresponding to x
d = des()
drawplot(key_var, texts_var)


# Different secret keys:


# Returns the Hamming Distances for given DES rounds of distict two plain text and single key
def helperfunchd(rounds1, rounds2, iham_dist):
    ans = [0]
    for x in range(16):
        ham_dist = 0
...
# This plots the box plot for different Rounds
def drawplot(keys, text):
    df = pd.DataFrame()
    for j in range(1, 6):
        r1, rounds1 = d.encrypt(keys[0], text)
        r2, rounds2 = d.encrypt(keys[j], text)
        ans = helperfunchd(rounds1, rounds2, j)
        df['HD'+str(j)] = ans
    df = df.T
    boxplot = df.boxplot()
